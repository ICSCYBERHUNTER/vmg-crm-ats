...
type FileType = CompanyDocument['file_type']
...
export function CompanyDocumentUploadForm({ onUpload, isFirstDocument, onValidationError }: Props) {
  const inputRef = useRef<HTMLInputElement>(null)
  const [pendingFile, setPendingFile] = useState<File | null>(null)
  const [fileType, setFileType] = useState<FileType>('fee_agreement')
      ...
      {pendingFile && (
        <div className="rounded-lg border border-border bg-muted/20 p-4 space-y-3">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            <Input
              placeholder="Notes (optional) — e.g., 2025 renegotiation — new fee structure"
              value={notes}
