-- Phase 2X.1.6 (Lever B): Replace AND-based tsquery with OR-based tsquery
-- Date: 2026-05-12
--
-- Problem: plainto_tsquery('english', 'sales engineer ot midwest') returns
-- 'sale' & 'engin' & 'ot' & 'midwest' — ALL tokens must be present in a
-- candidate's search_vector for the keyword branch to match. Natural-language
-- recruiter queries include framing words ("presales", "experience", "lives",
-- "midwest") that are absent from most candidates' stored text, so the AND
-- requirement silently zeroes out the keyword branch for multi-word queries.
--
-- Fix: replace every ' & ' separator with ' | ' so the tsquery uses OR
-- semantics. A candidate now enters the keyword branch if ANY meaningful
-- token matches. ts_rank still orders results within the branch, so
-- candidates matching more tokens still rank higher.
--
-- Only the tsquery construction line changes. All LIMITs, DEFAULTs,
-- deduplication logic, and other branches are identical to Phase 2X.1.5.
