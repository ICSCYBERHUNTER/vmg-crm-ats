-- =============================================================================
-- Increase candidate branch limits in hybrid_search() from 20 → 30
--
-- Problem: with LIMIT 20 per branch (semantic + keyword), after deduplication
-- the candidate pool is at most ~30-35 unique rows — never fills the
-- result_limit = 50 outer cap when scope = candidate.
--
-- Fix: raise each candidate branch to LIMIT 30. After dedup, up to ~50 unique
-- candidates can reach the outer cap, giving Voyage rerank a full 50-candidate
-- pool to work with.
--
-- No scoring changes, no fusion changes, no app-layer changes.
-- =============================================================================
