-- =============================================================================
-- Add optional filter_entity_type parameter to hybrid_search()
--
-- Allows the caller to restrict results to a single entity type at the SQL
-- level, eliminating cross-entity competition for the result_limit slots.
-- When NULL (the default), all branches run — identical to the prior version.
--
-- Because the new signature has a different argument count (5 vs 4),
-- CREATE OR REPLACE would create an overloaded function rather than replacing
-- the existing one. We must DROP the old 4-arg signature first.
-- =============================================================================

-- Step 1 — Drop the existing 4-arg function
DROP FUNCTION IF EXISTS public.hybrid_search(vector(1024), text, boolean, int);

-- Step 2 — Create the new 5-arg function
  ...
  WITH raw AS (
    ...
    (
      SELECT
        ...
        END::text,
        'semantic'::text,
        (1.0 - (j.embedding <=> query_embedding))::float,
        0::float,
        j.created_at
      FROM job_openings j
      WHERE j.embedding IS NOT NULL
        AND (filter_entity_type IS NULL OR filter_entity_type = 'job_opening')
    ...
    (
      SELECT
        ...
        'keyword'::text,
        0::float,
        ts_rank(j.search_vector, tsq)::float,
        j.created_at
      FROM job_openings j
      WHERE tsq IS NOT NULL
        AND j.search_vector @@ tsq
        AND (filter_entity_type IS NULL OR filter_entity_type = 'job_opening')
      ORDER BY ts_rank(j.search_vector, tsq) DESC
      LIMIT 10
    )
